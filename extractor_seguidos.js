// Extractor DOM para la vista de Cuentas Seguidas (Following)
// Resuelve virtualización de React mediante scroll asíncrono, micro-rebote y emisión de eventos sintéticos
(() => {
  const uiId = 'ig-following-ui';
  const styleId = 'ig-following-styles';
  ...
  ui.innerHTML = `
    <div class="ig-header">
      <span class="ig-title">Cuentas Seguidas</span>
  ...
  const getDialogAndScroller = () => {
    const dialog = document.querySelector('div[role="dialog"]');
    if (!dialog) return { dialog: null, scroller: null };

    const scroller = Array.from(dialog.querySelectorAll('div')).find(el => {
      const style = window.getComputedStyle(el);
      const isScroll = style.overflowY === 'auto' || style.overflowY === 'scroll';
      return isScroll && el.scrollHeight > el.clientHeight;
    });
  ...
  (async () => {
    const { dialog, scroller } = getDialogAndScroller();
    ...
    const blacklist = new Set(['explore', 'reels', 'direct', 'stories', 'p']);

    const harvest = () => {
      const links = scroller.querySelectorAll('a[role="link"], a[href^="/"]');
      links.forEach(a => {
        const href = a.getAttribute('href');
        if (href) {
          const handle = href.replace(/^\//, '').split('/')[0].split('?')[0].trim();
          if (handle && /^[a-zA-Z0-9._]{1,30}$/.test(handle) && !blacklist.has(handle.toLowerCase())) {
    ...
    while (stagnant < maxStagnant) {
      ...
      if (usernames.size === prevCount) {
        stagnant++;
        statusEl.textContent = `Esperando red (${stagnant}/${maxStagnant})...`;
    ...
    btnDl.onclick = () => {
      ...
      a.href = url;
      a.download = `seguidos_${Date.now()}.txt`;
