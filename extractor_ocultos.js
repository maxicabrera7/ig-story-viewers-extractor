// Extractor DOM para la vista de privacidad (Ocultar historia a)
// Requiere ejecución en: https://www.instagram.com/accounts/privacy_and_security/
(() => {
  const uiId = 'ig-hidden-story-ui';
  const styleId = 'ig-hidden-story-styles';
  ...
  ui.innerHTML = `
    <div class="ig-header">
      <span class="ig-title">Ocultos de Historia</span>
  ...
  const getExpandButton = () => {
    return Array.from(document.querySelectorAll('button, div[role="button"], span[role="button"], span')).find(el => {
      const txt = (el.textContent || '').trim().toLowerCase();
      const rect = el.getBoundingClientRect();
      return rect.left > 300 &&
             /mostrar.*restante|\d+\s+restante|ver más|cargar más|show.*remaining/i.test(txt) &&
             el.children.length < 3;
    });
  };

  const getMainScroller = () => {
    const scrollable = Array.from(document.querySelectorAll('div, main, section')).filter(el => {
      const rect = el.getBoundingClientRect();
      const style = window.getComputedStyle(el);
      const hasScroll = (style.overflowY === 'auto' || style.overflowY === 'scroll') && el.scrollHeight > el.clientHeight;
      return hasScroll && rect.left > 320;
    });
    return scrollable.length > 0 ? scrollable[scrollable.length - 1] : window;
  };

  const blacklist = new Set([
    'buscar', 'search', 'configuracion', 'configuración', 'ocultar', 'historia',
    'historias', 'videos', 'fotos', 'directo', 'personas', 'concretas',
    'seguidores', 'siguiendo', 'ver', 'mostrar', 'restantes', 'mensajes', 'inicio',
    'explorar', 'reels', 'direct', 'notificaciones', 'crear', 'panel', 'más', 'mas'
  ]);
  ...
  const harvest = () => {
    const candidates = Array.from(document.querySelectorAll('span, div')).filter(el => {
      ...
      if (rect.top < 130) return false;
      if (rect.bottom > window.innerHeight - 60 && rect.right > window.innerWidth - 200) return false;
      if (rect.width === 0 || rect.height === 0) return false;

      if (!/^[a-zA-Z0-9._]{1,30}$/.test(txt)) return false;
      if (blacklist.has(txt.toLowerCase())) return false;
    ...
    candidates.forEach(el => {
      usernames.add(el.textContent.trim().toLowerCase());
    });
  ...
  (async () => {
    let expandBtn = getExpandButton();
    let batch = 1;
    while (expandBtn) {
      statusEl.textContent = `Expandiendo lote ${batch}...`;
      expandBtn.scrollIntoView({ behavior: 'smooth', block: 'center' });
      expandBtn.click();
      batch++;
      await wait(1800);
      expandBtn = getExpandButton();
    }
    ...
    btnDl.onclick = () => {
      ...
      a.href = url;
      a.download = `historias_ocultas_${Date.now()}.txt`;
