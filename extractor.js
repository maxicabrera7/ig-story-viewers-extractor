(() => {
  const uiId = 'ig-viewer-extractor-ui';
  const styleId = 'ig-viewer-extractor-styles';
  ...
  styleSheet.textContent = `
    ...
    .ig-count.bump {
      animation: ig-pop 0.15s ease-out;
    }
    .ig-status {
      font-size: 12px;
      color: #71717a;
  ...
  (async () => {
    ...
    if (!scrollContainer) {
      statusEl.textContent = 'Buscando botón...';
      const openBtn = Array.from(document.querySelectorAll('div[role="button"], span, button')).find(el => {
        const text = el.textContent || '';
        return /Visto por|Vista por|Seen by|Viewers/i.test(text) && el.children.length < 5;
      });

      if (openBtn) {
        openBtn.click();
        statusEl.textContent = 'Abriendo lista...';
      } else {
        statusEl.textContent = 'Abre la lista de vistos...';
      }

      const startTime = Date.now();
      while (!scrollContainer && Date.now() - startTime < 15000) {
        await wait(500);
      ...
      if (!scrollContainer) {
        ...
        statusEl.textContent = 'Error: Modal no detectado';
        return;
      }
    }
    ...
    while (stagnantRounds < maxStagnant) {
      const links = scrollContainer.querySelectorAll('a[role="link"], a[href^="/"]');
      for (let i = 0; i < links.length; i++) {
        const href = links[i].getAttribute('href');
        if (href) {
          const handle = href.replace(/\//g, '').split('?')[0].trim();
      ...
      if (atBottom && stagnantRounds >= 2) break;
    ...
    btnCopy.onclick = async () => {
      try {
        ...
        btnCopy.querySelector('span').textContent = 'Error al acceder al portapapeles';
      }
    };
    ...
    btnDl.onclick = () => {
      ...
      a.href = url;
      a.download = `espectadores_${Date.now()}.txt`;
