// Extractor DOM para la vista de Cuentas Restringidas (Meta Bloks Web)
// Requiere ejecución en: https://www.instagram.com/accounts/privacy_and_security/ (sección Restringidas)
(() => {
  const uiId = 'ig-restricted-accounts-ui';
  const styleId = 'ig-restricted-accounts-styles';
  ...
  ui.innerHTML = `
    <div class="ig-header">
      <span class="ig-title">Cuentas Restringidas</span>
  ...
  // Localiza el scroll container de Bloks visible en el inspector
  const getScroller = () => {
    const candidates = Array.from(document.querySelectorAll('div')).filter(el => {
      const rect = el.getBoundingClientRect();
      if (rect.left < 250) return false; // Ignora sidebar
  ...
  const harvest = () => {
    // 1. Localizar los botones de acción como ancla
    const buttons = Array.from(document.querySelectorAll('*')).filter(el => {
      if (el.children.length > 0) return false;
      const txt = (el.textContent || '').trim();
      return /^quitar restricci[oó]n$/i.test(txt) || /^unrestrict$/i.test(txt);
    });

    buttons.forEach(btn => {
      // 2. Subir por el árbol hasta dar con la fila (que contiene el avatar)
      ...
      // 3. Extraer los textos de la fila (el handle siempre precede al nombre completo)
      const textNodes = Array.from(row.querySelectorAll('*'))
        .filter(el => {
          if (el.children.length > 0) return false;
          const t = el.textContent.trim();
          return t.length > 0 && !/^quitar restricci[oó]n$/i.test(t) && !/^unrestrict$/i.test(t);
        })
        .map(el => el.textContent.trim());

      // El primer nodo alfanumérico válido es el handle de la cuenta
  ...
  (async () => {
    ...
    // Rebobinar al inicio si el contenedor ya tenía scroll
    if (scroller !== window && scroller.scrollTop > 0) {
      statusEl.textContent = 'Rebobinando al inicio...';
    ...
    // Barrido descendente para resolver el lazy-loading de Bloks
    while (stagnant < 4) {
      if (scroller === window) {
    ...
    btnDl.onclick = () => {
      ...
      a.href = url;
      a.download = `cuentas_restringidas_${Date.now()}.txt`;
